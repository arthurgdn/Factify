...
export default ({postsList,startDownvote,startUpvote,displayVote=true})=>{
    ...
    return (
        ...
        {postsList.slice(0,postsList.length>feedIndex?feedIndex:postsList.length).map(({_id,author,title,content,hasVoted,score,createdAt})=>(
            ...
            {(postsList.length>feedIndex)&&(<button className="form__button" onClick={()=>setFeedIndex(feedIndex+10)}>Montrer plus</button>)}
        </div>
    )
}